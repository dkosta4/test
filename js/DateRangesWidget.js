			...
			'lastweeks' : {
				...
				dates: function() {
					...
					var monday = internal.getMonday(new Date());
                                        /* Mithun Kumar
                                         * 
                                         * Making last week starts from sunday(Monday - 1(day)) as we are making sunday as week starting day 
                                         */
...
//			}
	...
	var internal = {
		...
		refreshForm : function() {
                        ...
                        /* Mithun Kumar */
                        // Diabling future dates 
                        $('.datepickerFuture').addClass('datepickerDisabled');
...
//			$('.dr[lastSel=' + lastSel + ']', $dropdown).addClass('active');
		...
		createElements : function($target) {
			...
			$target.html(
				'<div class="date-range-field" style="margin-left: -13px;">'+
			...
			if (!$dropdown) {
				$dropdown = $(
				'<div id="datepicker-dropdown" onclick="textFocus();">'+
						...
						'<div class="main-daterange">'+
							'<div>'+
								...
								'<span class="parameters">'+
									'<input type="number" class="daterange-preset-parameter1" id="incId" autofocus/>'+
								'</span>'+
							'</div>'+
							'<input type="text" readonly="true" class="dr dr1 from" lastSel="0" placeholder="Start Date"/>   <input type="text" readonly="true" class="dr dr1 to" lastSel="1" placeholder="End Date"/>'+
...
//           			'</div>'+
						...
						'<button class="btn btn-mini btn btn-primary" id="button-ok" style="margin:5px">Apply</button>'+
						'<button class="btn btn-mini btn-primary"  id="button-cancel" style="margin:5px">Cancel</button>'+
						'</div>'+
					'</div>'+
				'</div>');
				...
				$datepicker.DatePicker({
					mode: 'tworanges',
                                         ...
                                         * Mithun Kumar 
                                         * 
                                         * Making the week starts from sunday instead of Monday
                                         * 
                                         */
					starts: 0,
					calendars: 3,
				...
				$daterangePreset.change(function() {
                                    ...
                                    /* Mithun Kumar
                                     * On Change of daterangePreset irrespective of the value setting parameter1 to 1
                                     * to address the following issue from internal testing team
                                     * Rel170717:Revenue:Increamental value next to Last Week(s),Last Month(s) and Last Quarters(s) ramains same.
                                     * Rel170717:Revenue:Calander in Last Day(7s) highlights the dates selected in Last Week(s).
                                     **/
				...
				$parameter1.change(function() {
					...
					if (!$.isNumeric(p1) || p1 < 1)
						internal.setParameter1(1);
                                        ...
                                        /* Mithun Kumar 
                                         * Based on selection not allowing values beyond the limits 4,12,52 for quarters , months, weeks respectively
                                         * */    
                                        var date_preset = $daterangePreset.val();
                                        if(date_preset == 'lastquarters')
                                        {
                                            if(p1 > 4)
                                            internal.setParameter1(4);
                                        }
                                        else if(date_preset == 'lastmonths')
                                        {
                                            if(p1 > 12)
                                            internal.setParameter1(12);
                                        }
                                        else if(date_preset == 'lastweeks')
                                        {
                                            if(p1 > 52)
                                            internal.setParameter1(52);
                                        }
				...
				$('#button-ok', $dropdown).click(function() {
                                        ...
                                        /* Mithun Kumar
                                         * Not allowing dates beyond 1 year duration
                                         * Taking 365 days as limit
                                         *         */
                                        internal.saveValues($current_target);
                                        var values = $current_target.data("DateRangesWidget").options.values;
                                        
                                        var minutes = 1000 * 60;
                                        var hours = minutes * 60;
                                        var days = hours * 24;
                                        var toDateArr =    values.dr1to.toString().split('/');
                                        var fromDateArr =    values.dr1from.toString().split('/');
                                        
                                        var toDate = new Date(toDateArr[2], parseInt(toDateArr[1])-1, toDateArr[0])
                                        var fromDate = new Date(fromDateArr[2], parseInt(fromDateArr[1])-1, fromDateArr[0])
                                        
                                        var diffInDays = (toDate.getTime() - fromDate.getTime())/days;
//                                        alert(toDate +"--"+ fromDate + "\n"+ toDate.getTime() + "--"+ fromDate.getTime() + " \n diff in milli seconds "+ (toDate.getTime() - fromDate.getTime()) + " \n days " + (toDate.getTime() - fromDate.getTime())/days);
//                                        alert(diffInDays);
                                        if(diffInDays > 365)
                                        {
                                            //$('#myModal').html('');
                                            $('#errMessageMaster').html(' More than one year is not allowed');
                                            $('#errMessageSuggAction').html('Please select within 1 year dates');
                                            $('#myModal').modal('show');
                                        }
                                        else
                                        {
                                        ...
                                        if ($('span.main').html().length == 22)
                                        {
                                            $(".date-range-field").css('margin-left', '-12px');
                                        } else if ($('span.main').html().length == 21)
                                        {
                                            $(".date-range-field").css('margin-left', '-3px');
                                        }else if ($('span.main').html().length == 23)
                                        {
                                            $(".date-range-field").css('margin-left', '-22px');
                                        }else if($('span.main').html().length == 19){
                                            $(".date-range-field").css('margin-left', '17px');
                                        } 
                                        else if($('span.main').html().length == 20){
                                            $(".date-range-field").css('margin-left', '8px');
                                        } 
                                        return false;
				});
			...
			$target.addClass('DRWClosed');
                        ...
                        $('.date-range-field a', $target).html('<img width="38px" height="38px" alt="" src="../img/sidebar/Calendar.png">');
		},
...
function textFocus()
{
    ...
    if (isChromium !== null && isChromium !== undefined && vendorName === "Google Inc." && isOpera == false && isIEedge == false) {
        document.getElementById("incId").focus();
    }
}